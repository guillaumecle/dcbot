...
public class UserManager {
	 ...
	 * @author Clement
	 */
	public void initUsers(List<String> descriptionsList) {
		users.clear();
		for (String description : descriptionsList) {
			int index = 0;
			index = description.indexOf(" ");
			String username = description.substring(0, index);
//			System.out.println(username);
			String reste = description.substring(index +1, description.length());
			String[] tab = reste.split("\\$");
			String desc = tab[0];
			String conn = tab[2];
			String mail = tab[3];
			String share = tab[4];
...
//			System.out.println(Arrays.toString(tab));
			users.add(new User(username, desc, conn, mail, share, _bot));
		}
	}
