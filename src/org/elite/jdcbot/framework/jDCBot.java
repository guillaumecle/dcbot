...
public abstract class jDCBot extends InputThreadTarget {
	...
	private int _userSearchDuration;
	...
	public jDCBot(String botname, String botIP, int listenPort, String password, String description, String conn_type, String email,
			String sharesize, int uploadSlots, int downloadSlots, boolean passive, PrintStream outputLog, int userSearchDuration) {
		_botname = botname;
		_password = password;
		// remove this and put
		...
		_passive = passive;
		_userSearchDuration = userSearchDuration;
		log = outputLog;
	...
	public jDCBot(String botIP) {
		this("jDCBot", botIP, 9000, "", "<++ V:0.668,M:A,H:1/0/0,S:0>", "LAN(T1)1", "", "0", 1, 3, false, System.out, 10);
	}
	...
	public final void connect(String hostname, int port) throws Exception {
		...
		*/
		...
		ThreadRechercheUser tru = new ThreadRechercheUser(this, log);
		tru.start();
		
		log.println("Recherche des utilisateur ...");
		Thread.sleep(_userSearchDuration * 1000);
		
		tru.stop();
		um.initUsers(tru.getDescriptionsList());
		/*
...
//		quit();
	 ...
	 */
	final String ReadCommand() throws Exception {
